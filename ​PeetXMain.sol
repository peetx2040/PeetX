    function owner() external view returns (address) {
        if (_isRenounced) return address(0);
        return _owner;
    }

    function USDT() external view returns (address) { return USDT_ADDRESS; }
    function NonfungiblePositionManagerAddress() external view returns (address) { return VAULT_NFT_CONTRACT; }

    function Chainlink() external view returns (bool) {
        uint256 checkedCount = 0;
        uint256 limitInv = nextInvestmentId;
        for (uint256 i = investmentCursor; i < limitInv && checkedCount < 100; i++) {
            Investment memory inv = investments[i];
            if (!inv.processed && !inv.earlyWithdrawn && block.timestamp >= inv.startTime + inv.duration) return true;
            checkedCount++;
        }
        uint256 checkedTrdCount = 0;
        uint256 limitTrd = nextTradeId;
        for (uint256 j = tradeCursor; j < limitTrd && checkedTrdCount < 100; j++) {
            Trade memory trd = trades[j];
            if (!trd.processed && block.timestamp >= trd.startTime + trd.duration) return true;
            checkedTrdCount++;
        }
        return false;
    }
    ...
    function invest(uint256 amount, uint256 planId, address referrer) external nonReentrant {
        ...
        if (!isUser[msg.sender]) isUser[msg.sender] = true;
        ...
        uint256 profitPct = 1;
        uint256 dur = 1 days;
        if (planId == 2) { profitPct = 7; dur = 7 days; }
        else if (planId == 3) { profitPct = 20; dur = 20 days; }
        else if (planId == 4) { profitPct = 30; dur = 30 days; }
        ...
        investments[invId] = Investment({
            id: invId, investor: msg.sender, amount: amount, planId: planId,
            profitPercent: profitPct, startTime: block.timestamp, duration: dur,
            totalGenPaid: genPaid, processed: false, earlyWithdrawn: false
        });
    ...
    function claimLiquidity(uint256 invId) external nonReentrant {
        ...
        uint256 refundable = inv.amount > inv.totalGenPaid ? inv.amount - inv.totalGenPaid : 0;
        if (refundable > 0) {
    ...
    function openTrade(uint256 amount, uint256 durationOption, bool isBuy) external nonReentrant {
        ...
        trades[trdId] = Trade({
            id: trdId, trader: msg.sender, amount: amount, duration: dur,
            startTime: block.timestamp, isBuy: isBuy, entryPrice: entryPrice,
            processed: false, won: false
        });
